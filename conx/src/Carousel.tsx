...
export default function Carousel({ children, className, navButtonBgColor, navButtonFillColor }: CarouselProps) {
  ...
  return (
    <div className='relative'>
      ...
      <div className={`p-2 bnext tablet:p-4 rounded-full ${navButtonBgColor} opacity-80 absolute top-[50%] -translate-y-1/2 right-1 hover:opacity-100 cursor-pointer`} onClick={next}>
          <GrNext className={navButtonFillColor} />
      </div>

      <div className={`p-2 bprev tablet:p-4 rounded-full ${navButtonBgColor} opacity-80 absolute top-[50%] -translate-y-1/2 left-1 hover:opacity-100 cursor-pointer`} onClick={prev}>
